...
function displayTasks() {
  ...
  taskList.innerHTML = '';
  const today = new Date().toISOString().split('T')[0];
  console.log('Today:', today); // Debug ngày hiện tại
  let filteredTasks = [];
...
function editTask(id) {
  ...
  if (!trimmedTitle) { alert('Vui lòng nhập tiêu đề mới!'); return; }
  ...
  tempInput.focus();
  tempInput.click();
  const newDeadline = new Promise(resolve => { tempInput.onchange = () => resolve(tempInput.value); });
  newDeadline.then(value => {
    document.body.removeChild(tempInput);
    const today = new Date().toISOString().split('T')[0];
    if (value && value < today) { alert('Deadline không thể là ngày trong quá khứ!'); return; }
    fetch(`/tasks/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: trimmedTitle, desc: newDesc || '', deadline: value || currentTask.deadline }) })
      .then(response => { if (!response.ok) throw new Error('Lỗi từ server'); return response.json(); })
      .then(data => { tasks = data; displayTasks(); })
      .catch(error => alert('Có lỗi xảy ra: ' + error.message));
  });
